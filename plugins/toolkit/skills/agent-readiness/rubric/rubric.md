...
**Cap P5 at 3** when any of these holds: the reviewer receives the diff but not the step's own acceptance criteria (nothing then grades intent — a flawless diff against the wrong step passes both graders); a `PLAN.md` is written by an agent and reviewed by nobody; or a failing hard gate can be re-scoped or raised by the same agent that wants to continue, with no independent adjudication and no committed decision record.
